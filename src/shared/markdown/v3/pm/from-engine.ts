 ...
 * up to three-level nested marks e.g. `**bold _italic_**` / `*em **strong** em*` /
 * `**bold *em ~~strike~~ more* bold**` / `**a *b ~~c _d_ c~~ b* a**`; hard breaks engine-owned; snake_case underscores stay literal),
 ...
 * deeper (four+) / ambiguous nested marks / multi-line HTML still go through `from-mdast.ts` after
 ...
 * Simple `*` / `**` / `_` / `__` / `~~` / `` ` `` marks (incl. up to three-level
 ...
 * `` `inline code` ``) plus **up to three-level nesting** (e.g. `**bold _italic_**`,
 * `*em **strong** em*`, `**bold *em ~~strike~~ more* bold**`, `**a *b ~~c _d_ c~~ b* a**`, `` **`code`** ``), **simple inline links / images**
 ...
 * (`***`, same-delimiter stacks, four+-level nests), multi-line HTML, nested-bracket wiki,
 ...
 * kept as text — not `math_inline` nodes); simple flat / up-to-three-level marks
 ...
 * Refuses: nested `[` / `]` (caller), newlines, `***` / unmatched / four+-level nests,
...
/** Max nest depth (3 = outer + three nested, e.g. `**a *b ~~c _d_ c~~ b* a**`). */
const MAX_MARK_NEST = 3;
