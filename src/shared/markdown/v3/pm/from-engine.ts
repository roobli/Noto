 ...
 * up to eight-level nested marks e.g. `**bold _italic_**` / `*em **strong** em*` /
 * `**bold *em ~~strike~~ more* bold**` / `**a *b ~~c _d_ c~~ b* a**` / `**a *b ~~c _d `e` d_ c~~ b* a**` / `**v *w ~~x _y *z `a` z* y_ x~~ w* v**` / `**u *v ~~w _x *y _z `a` z_ y* x_ w~~ v* u**` / `**t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t**` / `**s _t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t_ s**`; hard breaks engine-owned; snake_case underscores stay literal),
 ...
 * deeper (nine+) / ambiguous nested marks / multi-line HTML still go through `from-mdast.ts` after
 ...
 * Simple `*` / `**` / `_` / `__` / `~~` / `` ` `` marks (incl. up to eight-level
 ...
 * `` `inline code` ``) plus **up to eight-level nesting** (e.g. `**bold _italic_**`,
 * `*em **strong** em*`, `**bold *em ~~strike~~ more* bold**`, `**a *b ~~c _d_ c~~ b* a**`, `**a *b ~~c _d `e` d_ c~~ b* a**`, `**v *w ~~x _y *z `a` z* y_ x~~ w* v**`, `**u *v ~~w _x *y _z `a` z_ y* x_ w~~ v* u**`, `**t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t**`, `**s _t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t_ s**`, `` **`code`** ``), **simple inline links / images**
 ...
 * (`***`, same-delimiter stacks, nine+-level nests), multi-line HTML, nested-bracket wiki,
 ...
 * kept as text — not `math_inline` nodes); simple flat / up-to-eight-level marks
 ...
 * Refuses: nested `[` / `]` (caller), newlines, `***` / unmatched / nine+-level nests,
...
/** Max nest depth (8 = outer + eight nested, e.g. `**s _t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t_ s**`). */
const MAX_MARK_NEST = 8;
