 ...
 * up to six-level nested marks e.g. `**bold _italic_**` / `*em **strong** em*` /
 * `**bold *em ~~strike~~ more* bold**` / `**a *b ~~c _d_ c~~ b* a**` / `**a *b ~~c _d `e` d_ c~~ b* a**` / `**v *w ~~x _y *z `a` z* y_ x~~ w* v**` / `**u *v ~~w _x *y _z `a` z_ y* x_ w~~ v* u**`; hard breaks engine-owned; snake_case underscores stay literal),
 ...
 * deeper (seven+) / ambiguous nested marks / multi-line HTML still go through `from-mdast.ts` after
 ...
 * Simple `*` / `**` / `_` / `__` / `~~` / `` ` `` marks (incl. up to six-level
 ...
 * `` `inline code` ``) plus **up to six-level nesting** (e.g. `**bold _italic_**`,
 * `*em **strong** em*`, `**bold *em ~~strike~~ more* bold**`, `**a *b ~~c _d_ c~~ b* a**`, `**a *b ~~c _d `e` d_ c~~ b* a**`, `**v *w ~~x _y *z `a` z* y_ x~~ w* v**`, `**u *v ~~w _x *y _z `a` z_ y* x_ w~~ v* u**`, `` **`code`** ``), **simple inline links / images**
 ...
 * (`***`, same-delimiter stacks, seven+-level nests), multi-line HTML, nested-bracket wiki,
 ...
 * kept as text — not `math_inline` nodes); simple flat / up-to-six-level marks
 ...
 * Refuses: nested `[` / `]` (caller), newlines, `***` / unmatched / seven+-level nests,
...
/** Max nest depth (6 = outer + six nested, e.g. `**u *v ~~w _x *y _z `a` z_ y* x_ w~~ v* u**`). */
const MAX_MARK_NEST = 6;
