...
export function blockFromSpan(span: BlockSpan): ProseNode {
  // Engine-owned IR → PM for leaf / plain paragraph+heading / link-def / simple footnote-def / simple quote (incl. nested plain) / flat or same-family nested list (any depth) / simple table (skip mdast).
