 ...
 * leaf / plain paragraph+heading / simple quote (incl. nested plain + lists-in-quotes + hard breaks + lazy nest + no-`>` lazy) / flat or same-family nested list (any depth, incl. hard breaks + unindented lazy soft-wrap) / simple footnote-def (incl. hard breaks) / simple table skip mdast (IR → PM via `pm/from-engine.ts`).
 *
