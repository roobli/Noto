...
| `simple-nineteen-level-nested-marks.md` | nineteen-level nested marks (`_h *i **j _k *l **m _n *o **p _q *r **s _t *u ~~v _w *x _y *z `a` z* y_ x* w_ v~~ u* t_ s** r* q_ p** o* n_ m** l* k_ j** i* h_`, `_h **i _j *k **l _m *n **o _p *em **bold ~~del _x *y _z *w _v *u _t `a` t_ u* v_ w* z_ y* x_~~ more** rest* p_ o** n* m_ l** k* j_ i** h_`) in para/heading/list/table/callout/footnote; `***` / same-delimiter / twenty+ stay dialect |
