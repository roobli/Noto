...
ragged tables, and heavy inline (deep / ambiguous nested marks, reference
links, bare autolinks, HTML, escapes, nested-bracket wiki) stay on dialect.
...
optional title; plain or simple-marked link text), and **simple wiki links**
(`[[target]]` / `[[target|alias]]` as literal text; decoration plugin owns
display) are engine-owned.
...
deeper mark nests / reference links / bare autolinks), then reconsider
...
simple-underscore-emphasis, simple-nested-marks, simple-inline-links, simple-wiki-links, simple-gfm-tables,
