...
**[@roobli/md](https://github.com/roobli/md)** v0.1.12 (“WYSIWYG-first markdown
engine for Noto”).
...
```
"@roobli/md": "github:roobli/md#v0.1.12"
```
...
7–15 in `@roobli/md` v0.1.12) owns hard-break → two spaces, list marker /
...
tables, simple footnote-defs, empty footnote-defs, CJK emphasis, hard-breaks (incl. in quotes + lazy nest)/images/empty-fence/escapes/table-align/ordered-start/fence-meta/html-inline. Intentional diffs (mixed-marker nested lists; setext-`---` closed in v0.1.12) are listed in `tests/fixtures/markdown-golden/README.md` and
...
| Intentional diffs documented in `markdown-golden/README.md` | Mixed-marker nested lists (excluded); setext-`---` closed (v0.1.12 + `simple-setext-dash-headings.md`) |
...
**Adapter spike landed (default-off).** `@roobli/md` v0.1.12 is the pinned
...
Phase 15 setext-`---` vs thematic-break parity (v0.1.12) are available on the flagged path.
...
http(s) autolinks**, **simple angle-bracket http(s) autolinks**, **simple www. autolinks**, **simple email autolinks** (bare + angle / mailto), and **simple wiki links** (`[[target]]` / `[[target|alias]]` as literal text; decoration
plugin owns display) are engine-owned.
...
deeper mark nests / HTML / escapes), then reconsider
...
simple-footnote-defs, empty-footnote-defs, simple-setext-dash-headings, and cjk-emphasis goldens landed.
Do **not** flip the product default yet.

**Noto `0.0.2-alpha.9`** shipped the adapter (#37) plus `@roobli/md` v0.1.1 quote/
callout parity (#38). Pin is now `@roobli/md` v0.1.12 (Phase 15 setext-`---` vs thematic-break on top of
