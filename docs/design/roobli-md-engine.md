...
ragged tables, and heavy inline (deep / ambiguous nested marks, bare
autolinks, HTML, escapes, nested-bracket wiki) stay on dialect.
...
images** (`[text][id]` / `[text][]` / `![alt][id]` / `![alt][]`), and **simple
wiki links** (`[[target]]` / `[[target|alias]]` as literal text; decoration
...
deeper mark nests / bare autolinks / HTML / escapes), then reconsider
...
simple-underscore-emphasis, simple-nested-marks, simple-inline-links, simple-reference-links, simple-wiki-links, simple-gfm-tables,
